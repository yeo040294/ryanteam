...
const Unreserved = ({ myRequest, navigate,collectItem }) => {
  ...
  let filteredData = myRequest.filter(x => x.status === 'notCollected' && x.userId === localStorage.getItem("userid"))
  ...
  return (
    <MDBTable>
      ...
      <MDBTableBody>
      {(filteredData.length !== 0) ? display : <div><br /><React.Fragment>This user has yet to have any item un-reserved by admin in SecondLove.</React.Fragment></div>}
